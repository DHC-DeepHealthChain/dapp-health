...
export default class AlertSelected extends Component {
  ...
  // 选择
  choose(item, itemOptions) {
    if (!this.state.hide) {
      this.out();
      this.chooseTimer = setTimeout(() => {
        this.callback(item, itemOptions);
      }, 200);
    }
  }
  ...
  show(
    ...
    const title = itemData.placeholder;
  ...
  renderItem(item, i) {
    return (
      <View key={i} style={styles.tipContentView}>
        <View
          style={{ height: 0.5, backgroundColor: "#a9a9a9", width: aWidth }}
        />
        <TouchableOpacity onPress={this.choose.bind(this, this.state.itemData, item)}>
